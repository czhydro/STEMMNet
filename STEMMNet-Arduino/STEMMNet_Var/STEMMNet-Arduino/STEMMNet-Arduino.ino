...
#define SN_ID "SN003004"
#define API_KEY "cqNdlPKbp64Gc9dBGclC"
...
{0x28, 0xD8, 0x44, 0xE, 0xF, 0x0, 0x0, 0xC7},
{0x28, 0xAB, 0x73, 0xD, 0xF, 0x0, 0x0, 0x5B},
{0x28, 0x6, 0xCE, 0x20, 0xF, 0x0, 0x0, 0x3},
{0x28, 0x1E, 0xFF, 0x20, 0xF, 0x0, 0x0, 0xB0},
...
void loop() {
  ...
  if ((currentTime.unixtime() % UPLOAD_INTERVAL_SECONDS == 0 || !sdCardFunctioning) ){
    ...
    if (currentTime.unixtime() % CONFIG_INTERVAL_SECONDS == 0) {
      // do clock config and interval config
